...
func (f *Face) Glyph(dot fixed.Point26_6, r rune) (
	...
	charInfo := f.findChar(r)
	if charInfo == nil {
		return image.Rectangle{}, nil, image.Point{}, 0, false
	}

	width := int(charInfo.width)
	height := int(charInfo.height)

	xOffset := int(charInfo.xOffset)
	yOffset := int(charInfo.yOffset)
	deviceWidth := int(charInfo.deviceWidth)

	mask = charInfo.mask

	x := dot.X.Round() + xOffset
	y := dot.Y.Round() - yOffset

	dr = image.Rectangle{
		Min: image.Point{
			X: x,
			Y: y - height - f.Descent,
		},
		Max: image.Point{
			X: x + width,
			Y: y,
...
func (f *Face) GlyphBounds(r rune) (bounds fixed.Rectangle26_6, advance fixed.Int26_6, ok bool) {
	...
	return fixed.R(0, -height-f.Descent, f.MaxWidth, 0), fixed.I(f.MaxWidth), true
}
...
func (*Face) Kern(r0, r1 rune) fixed.Int26_6 {
	return 0
}
